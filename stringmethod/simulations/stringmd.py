...
from stringmethod.utils.conformation import *
...
class StringIterationRunner(object):
        ...
        for point_idx, point in enumerate(self.string):
            ...
            if os.path.isfile(tpr_file):
                ...
                if self.iteration <= 10:
                    path_confout = abspath("{}/{}/{}/restrained/confout.gro".format(self.md_dir, self.iteration - 1, point_idx))
                else:
                    path_confout = closest_conformation(self.string[point_idx], self.iteration)
                in_file = abspath(path_confout)
    ...
    def _compute_new_string(self) -> bool:
        ...
        # introduce new point on string every ... iteration:
        if self.iteration%8 == 0 and len(scaled_string) <= 41:
            old_length = len(scaled_string)
            new_length = old_length + 1
            new_scaled_string = utils.change_string_length(
                scaled_string, 
        	new_length
            )
        else:
